...
upstream_default_branch="$(gh api repos/$SYNC_UPSTREAM_REPO --jq '.default_branch')"
...
upstream_head_sha="$(gh api repos/$SYNC_UPSTREAM_REPO/git/ref/$default_ref --jq '.object.sha')"
if [ -z "$upstream_head_sha" ]; then
  echo "::error::Unable to resolve upstream SHA for $SYNC_UPSTREAM_REPO@$upstream_default_branch." >&2
  exit 1
fi

if gh api repos/$SYNC_FORK_OWNER/$SYNC_FORK_REPO/git/ref/heads/$branch >/dev/null 2>&1; then
  fork_head_sha="$(gh api repos/$SYNC_FORK_OWNER/$SYNC_FORK_REPO/git/ref/heads/$branch --jq '.object.sha')"
  if [ "$fork_head_sha" = "$upstream_head_sha" ]; then
    echo "Reusable branch '$branch' already exists and is synchronized." >&2
  else
    gh api --method PATCH repos/$SYNC_FORK_OWNER/$SYNC_FORK_REPO/git/refs/heads/$branch \
      -f sha="$upstream_head_sha" \
      -f force=true >/dev/null
    echo "Reusable branch '$branch' was reset to '$SYNC_UPSTREAM_REPO@$upstream_default_branch'."
  fi
else
  gh api --method POST repos/$SYNC_FORK_OWNER/$SYNC_FORK_REPO/git/refs \
